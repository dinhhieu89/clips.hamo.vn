...
function ocmx_page_header($before = "<h2>", $after = "</h2>"){
	...
	<?php elseif($is_archive) :?>
		<?php echo $before; __("Archives", "ocmx"); $after ?>
	<?php elseif(is_404()) : ?>
		<?php echo $before; __("Page Not Found", "ocmx"); $after; ?>
	...
	<?php elseif (is_search()) : ?>
		<?php $before; __("Your Search Results for:", "ocmx"); ?> "<em><?php the_search_query(); ?></em>"<?php echo $after; ?>
	<?php elseif(is_tag()): 
		single_tag_title();
	elseif(is_page()) : 
...
function ocmx_pagination($container_class = "clearfix", $ul_class = "clearfix")
	{
		...
		if((get_option("posts_per_page") && $numposts !== 0) && $numposts > get_option("posts_per_page", "ocmx")) :
?>
