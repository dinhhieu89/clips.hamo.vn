...
class DP_Widget_Comments extends WP_Widget {
	...
	function DP_Widget_Comments() {
		...
		$this->WP_Widget( "dp-comments", __('(DeDePress) Recent Comments', 'dp'), $widget_ops, $control_ops );
...
function dp_list_comments_widget($args = '',$echo = false) {
	...
	if($comments) {
		foreach ($comments as $comment) {
			...
			if($show_date)
				$output .= '<span class="date">'.sprintf(__('%s ago', 'dp'), relative_time(get_comment_time('U', true))).'</span> ';
